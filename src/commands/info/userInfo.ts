...
export default new Command({
    ...
    const profilePicture: string = member.user.displayAvatarURL({ dynamic: true, size: 2048, format: "png" });
    const UserEmbed: MessageEmbed = new MessageEmbed()
    .setAuthor({
        name: member.user.username,
        iconURL: member.user.displayAvatarURL({ dynamic: true, size: 2048, format: "png" }),
    })
    .setColor("#ee6f71")
    ...
    .addFields(
        ...
        {
        name: "Created at",
        value: `${TimeFull.format("dddd, MMMM Do YYYY, h:mm:ss a")} (<t:${Math.round(member.user.createdTimestamp / 1000)}:R>)`,
        inline: false,
        },
        {
        name: "Joined at",
        value: `${TimeFullAt.format("dddd, MMMM Do YYYY, h:mm:ss a")} (<t:${Math.round(member.joinedTimestamp / 1000)}:R>)`,
        inline: false,
        }
