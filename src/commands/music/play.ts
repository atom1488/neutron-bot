...
export default new Command({
    ...
    run: async({ interaction, client }) => {
        ...
        if (!interaction.member.voice.channel) return interaction.followUp({ ephemeral: true, content: `You need to be in a voice channel to use this command.` });

        if (interaction.guild.me.voice.channel) {
            if (interaction.guild.me.voice.channelId != interaction.member.voice.channelId) return interaction.followUp({ ephemeral: true, content: `You are not in the same voice channel as the bot.` });
        }
        ...
        const song: Song = await queue.play(query, {
