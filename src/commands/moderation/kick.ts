...
import { ButtonInteraction, GuildMember, InteractionCollector, MessageActionRow, MessageButton, MessageComponentInteraction } from "discord.js";
...
export default new Command({
    ...
    run: async ({ interaction }) => {
        ...
        setTimeout(() => { kickInteraction.deleteReply().catch(() => {}) }, 10000);
        const collector: InteractionCollector<MessageComponentInteraction> = kickInteraction.channel.createMessageComponentCollector({ time: 10000 });
        collector.on('collect', async(button: ButtonInteraction) => {
            if (button.customId === "kickYes") {
                ...
                kickInteraction.followUp({ content: `${memberAya.user.tag} has been kicked by ${interaction.member.user.username}`});
