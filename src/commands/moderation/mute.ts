import { ButtonInteraction, GuildMember, InteractionCollector, Message, MessageActionRow, MessageButton, MessageComponentInteraction, User } from "discord.js";
...
export default new Command({
    ...
    run: async ({ interaction }) => {
        ...
        if(!interaction.guild.me.permissions.has('ADMINISTRATOR'))
            ...
            muteInteraction.followUp({ content: `:warning: Are you sure to mute ${memberAya.user.username}`, components: [row]})
            setTimeout(() => { muteInteraction.deleteReply().catch(() => {}) }, 10000);
            const collector: InteractionCollector<MessageComponentInteraction> = muteInteraction.channel.createMessageComponentCollector({ time: 10000 });
            collector.on('collect', async (button: ButtonInteraction) => {
                if (button.customId === 'muteYes')
                {
                    ...
                    interaction.channel.send({ content: `**<@${memberAya.id}>** was muted for ${time} seconds by **<@${interaction.member.id}>**`});
