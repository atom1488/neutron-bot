...
export default new Command({
    ...
    run: async ({ interaction, client }) => {
       ...
       if(currentChannel.nsfw){
        ...
        if(bannedWords.includes(tag)) { tag = "1girls"} 
        const random = Math.floor(Math.random() * 1000)
        axios.get(`https://gelbooru.com/index.php?page=dapi&s=post&q=index&tag=${tag}&limit=1000&json=1`, { headers: { "Content-Type":"application/json" } })
        .then(res => {
